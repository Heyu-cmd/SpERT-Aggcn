...
class BaseInputReader(ABC):
    def __init__(self, types_path: str, tokenizer: BertTokenizer, neg_entity_count: int = None,
        ...
        for i, (key, v) in enumerate(types['relations'].items()):
            relation_type = RelationType(key, i + 1, v['english'], v['chinese'], v['symmetric'])
...
class JsonInputReader(BaseInputReader):
    ...
    def _parse_document(self, doc, dataset) -> Document:
        ...
        jstanford = doc['stanford']
        jwords = jstanford['cuts']
        jheads = jstanford['stanford_head']
        # parse tokens
        doc_tokens, doc_encoding = _parse_tokens(jtokens, dataset, self._tokenizer)

        # parse words
        doc_words = _parse_words(jwords, jtokens, dataset)
        ...
        # create document
        document = dataset.create_document(doc_tokens, entities, relations, doc_encoding, doc_words, jheads)
...
class JsonPredictionInputReader(BaseInputReader):
    ...
    def _parse_dataset(self, dataset_path, dataset):
        documents = json.load(open(dataset_path))[0:100]
        for document in tqdm(documents, desc="Parse dataset '%s'" % dataset.label):
            self._parse_document(document, dataset)

    def _parse_document(self, document, dataset) -> Document:
        if type(document) == list:
            jtokens = [t for t in document[0]]
        elif type(document) == dict:
            jtokens = document['tokens']
            jtokens = [t for t in jtokens]
            jstan = document['stanford']
            jwords = jstan['cuts']
            jheads = jstan['stanford_head']
        else:
        ...
        doc_words = _parse_words(jwords, jtokens, dataset)
        # create document
        document = dataset.create_document(doc_tokens, [], [], doc_encoding, doc_words, jheads)
...
def _parse_words(jwords, sentence, dataset):
    doc_words = []
    for i, (start, end) in enumerate(jwords):
        # 因为bert会在句子最前面加上 cls的representation，所以全部的span都要+1
        phrase = sentence[start:end]
        span_start, span_end = start + 1, end + 1
        word = dataset.create_word(i, span_start, span_end, phrase)
