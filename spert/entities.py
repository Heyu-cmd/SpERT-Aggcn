...
class RelationType:
    def __init__(self, identifier, index, english_name, chinese_name, symmetric=False):
        self._identifier = identifier
        self._index = index
        self._short_name = english_name
        self._verbose_name = chinese_name
...
class EntityType:
    def __init__(self, identifier, index, english_name, chinese_name):
        self._identifier = identifier
        self._index = index
        self._short_name = english_name
        self._verbose_name = chinese_name
...
class Word:
    def __init__(self, wid: int, idx: int, span_start: int, span_end: int, phrase: str):
        """
        分词后， 有toekn组成的Word
    ...
    def __eq__(self, other):
        if isinstance(other, Token):
            return self._wid == other._tid
        return False

    def __hash__(self):
        return hash(self._wid)
...
class Document:
    def __init__(self, doc_id: int, tokens: List[Token], entities: List[Entity], relations: List[Relation],
                 encoding: List[int], words: List[Word], heads: List[int]):
...
class Dataset(TorchDataset):
    ...
    def create_word(self, idx, span_start, span_end, phrase) -> Word:
        word = Word(self._wid, idx, span_start, span_end, phrase)
        self._wid += 1
        return word

    def create_document(self, tokens, entity_mentions, relations, doc_encoding,words,heads) -> Document:
        document = Document(self._doc_id, tokens, entity_mentions, relations, doc_encoding,words,heads)
