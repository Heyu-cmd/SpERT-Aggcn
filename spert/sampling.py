...
from spert.tree import Tree, head_to_tree, tree_to_adj


def create_train_sample(doc, neg_entity_count: int, neg_rel_count: int, max_span_size: int,
                        rel_type_count: int):
    ...
    # words
    word_spans, word_masks = [], []
    for w in words:
        word_spans.append(w.span)
        word_masks.append(create_entity_mask(*w.span, context_size))

    # adj
    tree = head_to_tree(heads, word_size)
    adj_np = tree_to_adj(word_size, tree, False)
    word_masks = torch.stack(word_masks)
    adj = torch.from_numpy(adj_np)
    word_sample_mask = torch.ones(word_size, dtype=torch.bool)
    # positive entities
    ...
    return dict(encodings=encodings, context_masks=context_masks, word_masks=word_masks, adj=adj,
                entity_masks=entity_masks, entity_sizes=entity_sizes, entity_types=entity_types,
                ...
                word_sample_mask=word_sample_mask)
...
def create_eval_sample(doc, max_span_size: int):
    ...
    return dict(encodings=encodings, context_masks=context_masks, entity_masks=entity_masks, word_masks=word_masks,
                adj=adj, word_sample_mask=word_sample_mask,
