...
class SpERT(BertPreTrainedModel):
    ...
    def __init__(self, config: BertConfig, cls_token: int, relation_types: int, entity_types: int,
        ...
        self._adj_ner = args['adj_ner']
        self._adj_rc = args['adj_rc']
        ...
        if self._adj_ner:
            self.entity_classifier = nn.Linear(config.hidden_size * 3 + size_embedding, entity_types)
        else:
    ...
    def _forward_train(self, encodings: torch.tensor, context_masks: torch.tensor, entity_masks: torch.tensor,
                       ...
                       adj: torch.tensor, word_masks: torch.tensor, word_sample_mask: torch.tensor):
        # get contextualized token embeddings from last transformer layer
        context_masks = context_masks.float()
        h = self.bert(input_ids=encodings, attention_mask=context_masks)['last_hidden_state']
        word_rep_gcn = None
        if self._adj_rc or self._adj_ner:
            # batch_size * max_word_len * max_sentence_len
            m = (word_masks.unsqueeze(-1) == 0).float() * (-1e30)
            word_span_pool = m + h.unsqueeze(1).repeat(1, word_masks.shape[1], 1, 1)
            word_rep = word_span_pool.max(dim=2)[0]
            word_rep_gcn, adj_mask = self.aggcn(adj, word_rep, word_sample_mask)
            word_rep_gcn = word_rep_gcn.masked_fill(adj_mask, -1e30)
            word_rep_gcn = torch.max(word_rep_gcn, 1)[0]
        ...
        for i in range(0, relations.shape[1], self._max_pairs):
            ...
            chunk_rel_logits = self._classify_relations(entity_spans_pool, size_embeddings,
                                                        relations, rel_masks, h_large, i, word_rep_gcn)
    ...
    def _forward_inference(self, encodings: torch.tensor, context_masks: torch.tensor, entity_masks: torch.tensor,
                           ...
                           word_masks:torch.tensor,adj:torch.tensor,word_sample_mask:torch.tensor):
        # get contextualized token embeddings from last transformer layer
        context_masks = context_masks.float()
        h = self.bert(input_ids=encodings, attention_mask=context_masks)['last_hidden_state']
        word_rep_gcn = None
    ...
    def _classify_entities(self, encodings, h, entity_masks, size_embeddings, word_rep_gcn):
        ...
        if self._adj_ner:
            word_rep_gcn = word_rep_gcn.unsqueeze(1).repeat(1, entity_spans_pool.shape[1], 1)
            entity_repr = torch.cat([entity_ctx.unsqueeze(1).repeat(1, entity_spans_pool.shape[1], 1),
                                     entity_spans_pool, size_embeddings, word_rep_gcn], dim=2)
    ...
    def _classify_relations(self, entity_spans, size_embeddings, relations, rel_masks, h, chunk_start, word_rep_gcn):
        ...
        if self._adj_rc:
            word_rep_gcn = word_rep_gcn.unsqueeze(1).repeat(1, rel_ctx.shape[1], 1)
            rel_repr = torch.cat([rel_ctx, entity_pairs, size_pair_embeddings, word_rep_gcn], dim=2)
        else:
    ...
    def _filter_spans(self, entity_clf, entity_spans, entity_sample_masks, ctx_size):
        ...
        for i in range(batch_size):
            ...
            # 如果关系超过了25000个，进行随机的挑选
            if len(rels) > 25000:
                rels_masks = random.sample(list(zip(rels, rel_masks)), 25000)
                rels, masks = zip(*rels_masks)
                sample_masks = [1] * 25000
